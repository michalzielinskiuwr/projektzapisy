...
read_variables()
{
# TODO - file path - programmatically
    file="/vagrant/db_backups/../env/.env"
...
# poll - install psycopg2
python anonymize.py "${DATABASE_USER}" "${DATABASE_PASSWORD}" "$TEMP_DB_NAME" "${DATABASE_PORT}"
...
7za a -t7z -m0=lzma -mx=9 -mfb=64 -md=32m -ms=on -p"$DATABASE_DUMP_PASSWORD" "$OUTFILE_PATH_PROD" "$TEMP_DUMP_PATH_PROD"
